...
for (let i = 0; i < input.length; i++) {
  ...
  if (requiredFields.every(field => element.indexOf(field) !== -1)) {
    part1Result++

    const areValuesValid = element.split(' ').every(item => {
      const field = item.split(':')[0]
      const value = item.split(':')[1]
      const number = value.match(/\d+/) && value.match(/\d+/)[0]

      if (field === requiredFields[0]) {
        if (value.length === 4 && number && 1920 <= number && number <= 2002) return true
        return false
      } else if (field === requiredFields[1]) {
        if (value.length === 4 && number && 2010 <= number && number <= 2020) return true
        return false
      } else if (field === requiredFields[2]) {
        if (value.length === 4 && number && 2020 <= number && number <= 2030) return true
        return false
      } else if (field === requiredFields[3]) {
        if (value.length === 5 && value.indexOf('cm') !== -1 && number && 150 <= number && number <= 193 ) return true
        if (value.length === 4 && value.indexOf('in') !== -1 && number && 59 <= number && number <= 76 ) return true
        return false
      } else if (field === requiredFields[4]) {
        if (value.length === 7 && !!value.match(/#[a-f0-9]{6}/g)) return true
        return false
      } else if (field === requiredFields[5]) {
        if (value.length === 3 && ['amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth'].indexOf(value) !== -1) return true
        return false
      } else if (field === requiredFields[6]) {
        if (value.length === 9 && !!value.match(/[0-9]{9}/g)) return true
        return false
      } else if (field === 'cid') {
        return true
      }
